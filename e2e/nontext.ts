 ...
 *
 * The first check earns its place in this repo immediately. `style.css` defines
 * `--border-strong` under a comment reading "Control boundaries only (WCAG 2.1
 * SC 1.4.11)" — and then applies it to exactly one rule,
 * `select, input[type="text"], textarea`. Every `button.copy-btn` on the page
 * drew its edge from `--border` instead, which is the SURFACE divider used for
 * card outlines and table rules; measured against the panel behind it that was
 * 1.39:1 in dark and 1.09:1 in light, and on hover — where it swaps to
 * `--border-hover` — 2.22:1 and 1.32:1. Nothing had ever looked, because before
 * this file nothing in this repo could.
 *
 * The second check finds nothing here, and that is a measurement rather than an
 * omission: neither `style.css` nor the shared header's inline block contains a
 * single `content` declaration, so this page has no generated content to judge.
 * It runs at every driven state so the first one added is caught.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks nine sections, seven live KAT rows and — the
     * expensive part — the ARIA S-box grid's 256 cells and the avalanche
     * exhibit's 128 `.bit-cell` spans, all re-walking the same ancestors up to
     * `<body>`. Without the caches the pass re-reads the same computed styles and
     * rects tens of thousands of times. Nothing mutates the DOM during the pass,
     * so the cached values cannot go stale.
     */
